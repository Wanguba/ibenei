...
arrowBtns.forEach((btn, i) => {
  ...
  btn.onclick = () => {
    dir = (i == 0) ? 1 : -1;
    currentCard += (i === 0) ? -1 : 1;
    currentCard = Math.min(10, Math.max(0, currentCard));
    moveCards(0.75);
  };
});
...
function moveCards(dur = 0) {
  gsap.timeline({ defaults: { duration: dur, ease: 'power3', stagger: { each: -0.03 * dir } } })
    ...
    .to('.arrow-btn-prev, .arrow-btn-next', {
      autoAlpha: (i) => (i === 0 && currentCard === 0) || (i === 1 && currentCard === 9) ? 0 : 1
    }, 0)
    .to('.card h4', {
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const products = document.querySelectorAll('.pro'); // All products
  ...
  // Optional: Automatically filter when the user types
